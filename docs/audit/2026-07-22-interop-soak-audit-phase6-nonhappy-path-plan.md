...
- **F006 (BESTÄTIGT — SIP-Defekt, hoch, blockiert Gruppe B):** **Authentifizierte ausgehende Calls an Asterisk scheitern grundsätzlich** mit `481 Call/Transaction Does Not Exist` am **Auth-Retry-INVITE** (CSeq 2 nach 401-Challenge) — uniform, unabhängig von Transport und SRTP. **Code-Verifikation bestätigt die Ursache:** `SipCallSessionTransactionService.cs:223` setzt `_context.RemoteTag` aus dem To-Tag der 401-Fehlerantwort, `SipCallSessionHeaderService.cs:61-63` hängt ihn an den Retry-INVITE-`To` → syntaktisch in-dialog für einen nie etablierten Dialog → 481 (RFC-3261-Verletzung §12.1.2). REGISTER ist immun (`SipRegistrationService.cs:126` baut `To` einmalig) — das erklärt den Differenzierer. Der zuvor beobachtete `Connection refused` war ein **Nebenbefund** (SDK eskaliert den 1305-B-INVITE RFC 3261 §18.1.1-konform auf TCP; UDP-only-Fixture ohne TCP-Transport). Damit ist auch die **F005-Taxonomie-Prämisse** (Codes aus `.Error`) hinfällig — Calls erreichen den Dialplan nie. (Register F006.)

**Gruppe B, Fixture-Dialplan (`extensions.conf`) und die `InternalsVisibleTo`-Freischaltung bleiben zurückgestellt**, bis F006 gefixt ist (separates Fix-Paket) — danach direkt freischaltbar. **Fix-Ort:** `_context.RemoteTag` beim initialen INVITE-Auth-Retry nicht aus der 401-To-Tag setzen (Dialog erst mit 2xx). Repro für die Fortsetzung: `[transport-tcp]` in pjsip.conf (oder `SrtpPolicy.Disabled`) + Dialplan `[default]` (busy→Busy(), decline→Hangup(21), noanswer→Ringing()+Wait).
