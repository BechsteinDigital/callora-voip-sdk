...
- **F006 (in Untersuchung → blockiert Gruppe B):** Der ausgehende INVITE an Asterisk scheitert transport-seitig mit `SocketException: Connection refused` (synthetischer 503) VOR jeder SIP-Antwort; REGISTER an dieselbe IP:5060 funktioniert. Damit sind 486/603/404/no-answer/CANCEL derzeit nicht testbar und die ursprüngliche **F005-Taxonomie-Prämisse** (SIP-Codes aus `.Error` rückgewinnbar) empirisch nicht haltbar (Calls kommen nie an; REGISTER-`Error` ist zudem `null`). (Register F006.)

**Gruppe B, Fixture-Dialplan (`extensions.conf`) und die `InternalsVisibleTo`-Freischaltung wurden zurückgestellt**, bis F006 root-caused ist — sonst käme ungenutzte Infrastruktur / nie laufende Skip-Tests in den PR. Nächster Schritt: F006 per SDK-SIP-Logging bzw. Paket-Capture (an welche Adresse:Port geht der INVITE?) und gegen einen realen Registrar analysieren.
